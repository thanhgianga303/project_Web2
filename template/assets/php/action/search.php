...
//SELECT * FROM `product` WHERE TenSP LIKE '%a%' 
    ...
    <div id="grid-sidebar1" class="tab-pane fade active show">
        <div class="row">
            ...
            if ($tong_sp > 0) {
                ...
                $thucthi = mysqli_query($connect, "SELECT * FROM `product` WHERE TenSP LIKE '%$timkiem%'  ORDER BY `product`.`MaSP` LIMIT $start,$limit");
                while ($row = mysqli_fetch_array($thucthi)) {
                    ?>
    ...
    <ul id="showNumPage">
        ...
        if ($so_trang > 1) {
            if ($so_trang > 1) //Thì mới hiện thị Pre ( dấu '<')
            {
                ...
                if ($current_page != 1) {
                    ?>
                    <li><a onclick="search_sp('<?php echo $pre ?>')"><i class="ti-angle-left"></i></a></li>
                <?php
            }
        }
        //$page = $_GET['page'];
        ...
        for ($i = 1; $i <= $so_trang; $i++) {
            if ($page == $i) {
                ...
                ?>
                    <li><a href="javascript:void(0)" id="list" onclick="search_sp(<?php echo $i ?>)"><?php echo $i ?></a></li>
                <?php
            }
        }
        if ($so_trang > 1) //Thì mới hiện thị Pre ( dấu '>')
        {
            ...
            if ($current_page != $so_trang) {
                ?>
                    <li><a onclick="search_sp('<?php echo $next ?>')"><i class="ti-angle-right"></i></a></li>
                <?php
            }
        }
    }
